...
use sdkwork_music_generation_mcp_service::{
    streamable_http_service, GenerateMusicInput, MusicGenerationMcpService,
};
use sdkwork_music_generation_service::{
    MusicGenerationCommand, MusicGenerationProviderDescriptor, MusicGenerationProviderResult,
    MusicGenerationServicePort, MusicProviderDispatchPlan, MusicProviderSubmission,
...
struct FakeService;
#[async_trait]
impl MusicGenerationServicePort for FakeService {
    async fn generate(
        &self,
        _: MusicGenerationCommand,
    ) -> MusicGenerationProviderResult<MusicProviderSubmission> {
        unreachable!()
    }
    async fn retrieve(
        &self,
        _: &MusicProviderDispatchPlan,
        _: &str,
...
fn service() -> MusicGenerationMcpService {
    MusicGenerationMcpService::new(Arc::new(FakeService))
}
#[test]
...
fn unified_input_maps_vendor_extensions_without_sdk_dtos() {
    let input: GenerateMusicInput = serde_json::from_value(serde_json::json!({
        "vendor": "Example_Vendor",
        "model": "music-model",
        "prompt": "minimal piano",
        "vendorParameters": { "schema": "urn:example:music:v1", "values": { "seed": 7 } }
    }))
    .unwrap();
    let command: MusicGenerationCommand = input.try_into().unwrap();
    assert_eq!(command.vendor.as_str(), "example-vendor");
    assert_eq!(
        command.vendor_parameters.unwrap().schema,
        "urn:example:music:v1"
    );
}
