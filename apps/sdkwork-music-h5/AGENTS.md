# SDKWork Music H5 Application Guidelines
...
This is the H5 application root for SDKWork Music. It follows `APP_H5_ARCHITECTURE_SPEC.md` for root layout, package taxonomy, SDK/IAM boundary, and verification style. H5 is the canonical phone-first mobile web runtime.

## Local Dictionary Structure

- `AGENTS.md`: local agent entrypoint and relative SDKWORK spec index.
- `sdkwork.app.config.json`: application manifest for H5 surface.
...
- `config/`: architecture-local configuration for browser, host, server, and container environments.
- `src/`: thin application bootstrap, providers, route assembly, and mobile shell entry.
- `packages/`: SDKWork H5 packages for app, console, and admin capabilities.
