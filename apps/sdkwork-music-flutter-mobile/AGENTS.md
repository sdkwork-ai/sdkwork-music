# SDKWork Music Flutter Mobile Application Guidelines
...
This is the Flutter mobile application root for SDKWork Music. It follows `FLUTTER_APP_MOBILE_ARCHITECTURE_SPEC.md` for root layout, package taxonomy, config, host/platform boundary, commands, release, and architecture verification.

## Local Dictionary Structure

- `AGENTS.md`: local agent entrypoint and relative SDKWORK spec index.
- `sdkwork.app.config.json`: application manifest for Flutter mobile surface.
- `.sdkwork/`: local skills, plugins, manifests, and agent workspace metadata.
- `specs/`: local contracts that narrow root standards.
- `sdks/`: SDK families for this application surface.
- `config/`: architecture-local configuration for app, host, server, and container environments.
- `lib/`: thin Flutter app bootstrap, providers, shell assembly, route registry, SDK client construction, IAM runtime wiring, and platform adapter registration.
- `packages/`: SDKWork Flutter mobile packages for app, console, and admin capabilities.
- `test/`: cross-package tests, contract tests, integration tests, and fixtures.
...
- Dart/Flutter code: `../../sdkwork-specs/APP_FLUTTER_UI_SPEC.md`.
...
For Dart/Flutter code, prefer strict types, explicit package exports, colocated tests, and existing package/module boundaries.
...
- `flutter analyze`: run Dart analysis checks.
- `flutter build`: build the application for target platform.
