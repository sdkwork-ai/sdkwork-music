# SDKWork Music PC Application Guidelines
...
This is the PC application root for SDKWork Music. It follows `APP_PC_ARCHITECTURE_SPEC.md` for root layout, package taxonomy, SDK/IAM boundary, app/console/admin separation, and verification style.

## Local Dictionary Structure

- `AGENTS.md`: local agent entrypoint and relative SDKWORK spec index.
- `sdkwork.app.config.json`: application manifest for PC surface.
...
- `config/`: architecture-local configuration for browser, desktop, server, and container environments.
- `src/`: thin application bootstrap, providers, route assembly, and layout entry.
- `packages/`: SDKWork PC packages for app, console, and admin capabilities.
